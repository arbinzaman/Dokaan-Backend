  ...
  class DokaanController {
    ...
    static async getById(req, res) {
      try {
        ...
        if (!dokaan) {
          return res.status(404).json({ status: 404, message: "Dokaan not found" });
        }
        return res.json({ status: 200, data: dokaan });
      } catch (error) {
    ...
    static async addEmployee(req, res) {
      try {
        ...
        return res.json({ status: 200, message: "Employee added to Dokaan", data: employee });
      } catch (error) {
    ...
    static async removeEmployee(req, res) {
      try {
        ...
        return res.json({ status: 200, message: "Employee removed from Dokaan", data: employee });
      } catch (error) {
    ...
    static async addEmployeeFromExistingAccount(req, res) {
        try {
          ...
          const employee = await addEmployeeFromExistingAccount(email, dokaanId, shopRole);
